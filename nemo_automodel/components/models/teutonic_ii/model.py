...
def eager_attention_forward(
    ...
    **kwargs: Any,
    ...
    del kwargs
    num_key_value_groups = cast(int, module.num_key_value_groups)
    ...
    if sinks is not None:
        attention_sink_bias = cast(torch.Tensor, module.attention_sink_bias)
        sink_bias = attention_sink_bias.reshape(1, -1, 1, 1).expand(
...
class TeutonicIIAttention(nn.Module):
    """Teutonic-II hybrid attention (full or sliding-window)."""
    ...
    def __init__(
        ...
        if projection_layout not in {"split", "fused_qkv"}:
            raise ValueError(
                f"Unsupported TeutonicII attention projection layout: {projection_layout}"
            )
        ...
        if self.rope_dim % 2 != 0:
            raise ValueError(
                f"TeutonicII rotary dimension must be even, got {self.rope_dim} from "
        ...
        if has_sink:
            self.register_buffer(
                "attention_sink_bias",
                torch.empty(self.num_attention_heads, dtype=torch.float32),
            )
        else:
...
class TeutonicIIRotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if is_swa:
            ...
            self.config.head_dim = cast(
                int, getattr(config, "swa_head_dim", getattr(config, "head_dim", None))
            )
    ...
    def compute_default_rope_parameters(
        ...
        if dim % 2 != 0:
            raise ValueError(
                f"TeutonicII rotary dimension must be even, got {dim} from "
...
class TeutonicIIDecoderLayer(nn.Module):
    def __init__(
        self,
        config: TeutonicIIConfig,
        layer_idx: int,
        ...
        self.self_attn = TeutonicIIAttention(
            config,
            is_swa=is_swa_layer,
            layer_idx=layer_idx,
        ...
        moe_layer_freq = cast(list[bool], config.moe_layer_freq)
        is_moe_layer = (
            getattr(config, "n_routed_experts", None) is not None
            and moe_layer_freq[layer_idx]
        )
        self.mlp = (
    ...
    def init_weights(self, buffer_device: torch.device) -> None:
        del buffer_device
        attention_sink_bias = getattr(self.self_attn, "attention_sink_bias", None)
        if attention_sink_bias is not None:
            nn.init.zeros_(attention_sink_bias)


class TeutonicIIModel(nn.Module):
    def __init__(
        self, config: TeutonicIIConfig, moe_config: MoEConfig, backend: BackendConfig
    ):
        super().__init__()
        self.config = config
        ...
        self.layers = nn.ModuleDict(
            {
                str(i): TeutonicIIDecoderLayer(config, i, moe_config, backend)
        ...
        self.rotary_emb = TeutonicIIRotaryEmbedding(config=config, is_swa=False)
        self.swa_rotary_emb = TeutonicIIRotaryEmbedding(config=config, is_swa=True)
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        for layer in self.layers.values():
            ...
            hidden_states = layer(
                hidden_states,
                attention_mask=causal_mask_mapping[cast(str, layer.attention_type)],
    ...
    def init_weights(self, buffer_device: Optional[torch.device] = None) -> None:
        ...
        with buffer_device:
            ...
            cast(nn.Module, self.norm).reset_parameters()
        for layer in self.layers.values():
            cast(Any, layer).init_weights(buffer_device)


class TeutonicIIForCausalLM(HFCheckpointingMixin, nn.Module, MoEFSDPSyncMixin):
    """Teutonic-II causal LM wrapper."""
