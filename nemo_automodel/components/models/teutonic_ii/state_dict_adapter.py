...
class TeutonicIIStateDictAdapter(MoESplitExpertsStateDictMixin, StateDictAdapter):
    """Convert Teutonic-II HF checkpoints to Automodel's grouped MoE layout."""
    ...
    def convert_single_tensor_to_hf(
        ...
        expert_result = self._convert_single_merged_expert_to_hf_split_experts(
            fqn, tensor, **kwargs
        )
        ...
        if exclude_key_regex:
            result = [
                ...
                if not re.match(exclude_key_regex, key)
            ]
        ...
        for key, value in result:
            if _should_quantize_key(key):
                ...
                quantized_result.append(
                    (key + "_scale_inv", create_scale_inv_for_weight(quantized))
                )
            else:
    ...
    def _dequantize(self, state_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        for key in list(state_dict.keys()):
            ...
            if key.endswith(".self_attn.qkv_proj.weight"):
                state_dict[key] = self._dequantize_interleaved_qkv(
                    state_dict[key], scale_inv, key
                )
            else:
        ...
        logger.debug(
            "[Teutonic II FP8 Dequant] Dequantized %s weights", dequantized_count
        )
        return state_dict
    ...
    def _dequantize_interleaved_qkv(
        ...
        if match is None:
            raise ValueError(f"Cannot determine TeutonicII layer index from {key}")
        ...
        full_scale = (
            scale_inv.full_tensor() if isinstance(scale_inv, DTensor) else scale_inv
        )
        full_scale = full_scale.to(device=local_weight.device)
        expected_scale_shape = (
            ckpt_tp * scale_rows_per_shard,
            (weight.shape[1] + 127) // 128,
        )
        if full_scale.shape != expected_scale_shape:
            raise ValueError(
                f"{key} scale_inv has shape {full_scale.shape}, expected {expected_scale_shape}"
            )
        ...
        global_rows = (
            torch.arange(local_weight.shape[0], device=local_weight.device) + row_offset
        )
        scale_rows = (global_rows // rows_per_shard) * scale_rows_per_shard
        scale_rows = scale_rows + (global_rows % rows_per_shard) // 128
        scale_cols = (
            torch.arange(local_weight.shape[1], device=local_weight.device) + col_offset
        ) // 128
